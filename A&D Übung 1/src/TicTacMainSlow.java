...
public class TicTacMainSlow extends Frame
{
    ...
    private static class TicTacPanel extends Panel implements ActionListener
    {
        ...
        public void doCompMove( boolean thinkAboutIt )
        {
            ...
            if( thinkAboutIt )
                compMove = t.chooseMove( TicTacToeSlow.COMPUTER );
            else
            {
            ...
            t.playMove( TicTacToeSlow.COMPUTER, compMove.row, compMove.column );
        }
                ...
                for( int i = 0; i < 3; i++ )
                    for( int j = 0; j < 3; j++ )
                        if( evt.getSource( ) == squares[ i ][ j ] )
                            t.playMove( TicTacToeSlow.HUMAN, i, j );

                if( resetIfDone( t.boardIsFull( ), "DRAW", true ) )
                    return;
                doCompMove( true );           
                resetIfDone( t.isAWin( TicTacToeSlow.COMPUTER ), "I WIN!!", true );
